...
# displayed in help message
TESTSUITE_DESCRIPTION = "Design your own app - Intermediate assignment (Snake) testsuite - student version"
...
class TestCaseBase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        evaluation_delta = {
            i: EVALUATION[i] - self.initial_evaluation[i] for i in EVALUATION.keys()}
...
BOARD_HEADER_PATTERN = re.compile(
    r'^Lives: (-?\d+) - Apple Lives: (-?\d+) - Score: (-?\d+)')
        ...
        for width in [8]:
            for height in [8]:
                with self.subTest(BOARD_HEIGHT=height, BOARD_WIDTH=width):
                    ...
                    self.assertEqual(height + 5, len(output),
                                     "Incorrect number of lines in output")
                    self.assertEqual(
                        "-" * (width * 3 + 4), output[1], "Incorrect number of '-' above board.")
                    for row in range(height):
                        rl = rowletter(row)
                        self.assertEqual(
                            f'{rl} | ', output[row + 2][:4], f"Row {rl} starts incorrectly")
                        self.assertEqual(
                            f' |', output[row + 2][-2:], f"Row {rl} ends incorrectly")
                    self.assertEqual(
                        "-" * (width * 3 + 4), output[-3], "Incorrect number of '-' below board.")
                    self.evaluation(1, 7)

    def test_header_simple(self):
        with captured_io() as (_, out, _):
            LIBRARY._1_print_game_board()
        self.assertEqual(f'Lives: 3 - Apple Lives: 10 - Score: 0',
                         out.getvalue().split('\n')[0])
        self.evaluation(1, 1)
    ...
    def test_snake_head(self):
        for orientation_number, orientation in {
            ...
            with self.subTest(orientation=orientation[0]):
                ...
                self.assertEqual(
                    orientation[1], head, f"The snake head should be '{orientation[1]}' when looking {orientation[0]}. Board: \n" + out.getvalue())
    ...
    def test_snake_tail_line(self):
        ...
        for length in range(lower_bound, upper_bound + 1):
            with self.subTest("straight up", snake_length=length):
                with captured_io() as (_, out, _):
                    LIBRARY.SNAKE = [
                        rowletter(i) + '0' for i in range(length - 1, -1, -1)]
                ...
                for row in range(length - 1):
                    self.assertEqual(
                        '+', output[3 + row][4], f"Cell {rowletter(row)}0 is part of the snake tail, but not '+' on the following board:\n" + out.getvalue())
            ...
            with self.subTest("straight down", snake_length=length):
                ...
                for row in range(length - 1):
                    self.assertEqual(
                        '+', output[2 + row][4], f"Cell {rowletter(row)}0 is part of the snake tail, but not '+' on the following board:\n" + out.getvalue())
                self.evaluation(3, evaluation_score_per_subtest)

            with self.subTest("straight left", snake_length=length):
                with captured_io() as (_, out, _):
                    LIBRARY.SNAKE = ['A' + str(i)
                ...
                for col in range(length - 1):
                    self.assertEqual(
                        '+', output[2][7 + 3 * col], f"Cell A{col} is part of the snake tail, but not '+' on the following board:\n" + out.getvalue())
                self.evaluation(3, evaluation_score_per_subtest)
            ...
            with self.subTest("straight right", snake_length=length):
                ...
                for col in range(length - 1):
                    self.assertEqual(
                        '+', output[2][4 + 3 * col], f"Cell A{col} is part of the snake tail, but not '+' on the following board:\n" + out.getvalue())
                self.evaluation(3, evaluation_score_per_subtest)
...
class TestGame(TestCaseBase):
            ...
            with self.subTest():
                ...
                with captured_io('\n\nq\n') as (_, out, _):
                    try:
                        ...
                        pass
                self.assertEqual(
                    lives - 1, LIBRARY.LIVES, "LIVES should have been decremented. Output:\n" + out.getvalue())
                self.assertNotEqual(
                    apple, LIBRARY.APPLE, "The apple should have spawned at a new location. Output:\n" + out.getvalue())
    ...
    def test_game_over(self):
        ...
        self.assertEqual(2, out.getvalue().count(
            'input [w a s d]'), f"Don\'t prompt for a movement direction after LIVES equals 0. Output:\n" + out.getvalue())
        self.evaluation(6, 5)
        ...
        for i in range(1, 5):
            with captured_io('\n') as (_, out, _):
                try:
                    ...
                    pass
            self.assertEqual(score + i, LIBRARY.SCORE,
                             f"The score should have been {score + i}. Output:\n" + out.getvalue())
            self.evaluation(4, 3/4)


LEADERBOARD_ITEM_PATTERN = re.compile(
    r'.* - Score: (\d+) - Lives: (\d+) - Snake length: (\d+)')
...
class TestLeaderboard(TestCaseBase):
    def setUp(self):
        ...
        with open(HISTORY_FILE, 'r') as file:
            self.assertEqual(
                [], list(file), "The history file should have been cleared.")
...
class Test(TestCaseBase):
    def test_matnr(self):
        self.assertNotEqual("12345678", LIBRARY.MATRICULUM_NUMBER,
                            "Enter your matriculum number (in your submission).")
...
# If you are a student, don't worry about this variable
INITIAL_LIBRARY_ATTRIBUTES = {}
...
def captured_io(initial_in=None) -> Tuple[io.StringIO, io.StringIO, io.StringIO]:
    new_in, new_out, new_err = io.StringIO(
        initial_in), io.StringIO(), io.StringIO()
...
def test(lib, filename: Path):
    ...
    global EVALUATION
    print(
        f"Completed testing {filename} - evaluation: {', '.join(f'{k}: {v}/{m}' for (k, v), m in zip(EVALUATION.items(), [8, 0, 2.5, 5, 0, 16, 0]))} = {sum(EVALUATION.values())}/31.5")
...
# cli arg; Terminate the subprocess if importing the library takes longer
LOAD_TIMEOUT = None
# cli arg; Terminate the subprocess if takes longer to finish gracefully
...
def main():
    ...
    # key: pid, value: stdout & stderr for each script
    outputs: Dict[int, str] = {}
    exitcodes: Dict[int, int] = {}  # key: pid, value: exitcode

    timeouts: List[Tuple[int, datetime]] = []  # [(pid, datetime), (...), ...]

    ready: List[multiprocessing.Process] = list(map(lambda x: multiprocessing.Process(
        target=runtest, args=(*x, queue)), enumerate(scripts)))
    ...
    while len(ready) > 0 or len(running) > 0:
        ...
        # give timeouted processes enough time to report their output
        time.sleep(TERMINATION_DELAY.total_seconds())
        for pid, t in timeouts:
            ...
            for i, p in enumerate(running):
                if pid == p.pid and p.is_alive():
                    print(
                        f"process {pid} exceeded its timeout {t} by {datetime.now() - t}, terminating")
                    p.terminate()
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=TESTSUITE_DESCRIPTION, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
